...
- [r2r-jev](https://github.com/Thneoly/r2r-jev) — Agent governance: checks whether each tool call exceeds scope or is destructive, feeding those judgments into persistent trust and authorization state that can require human override.
- [Agent Chaperone](https://github.com/agent-chaperone/agent-chaperone) — Agent safety: screens tool calls before execution and tool results before the agent reads them through an MCP proxy and hooks.
...
- [GeekLink Jev Subtitle Translator](https://github.com/GeekLinkDev/jev-subtitle-translator) — Translation QA: reviews each translated subtitle line with Jev for omissions, changed meaning, names, numbers, negation, and other defects before export.
...
### Workflow & semantic gates

- [TryJevAI](https://tryjevai.com/) — Scheduling: distinguishes a mentioned time from an agreed meeting time with a Choice that includes `Unresolved`, prompting for clarification when the agreement is not established.
