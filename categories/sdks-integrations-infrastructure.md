...
- [kojev](https://github.com/ItisNoMatter/kojev) — Kotlin ecosystem: provides a Kotlin Multiplatform client for Choice and Score questions that maps answers into caller-defined enums without built-in thresholds.
- [ask-jev](https://github.com/logicrw/ask-jev) — Python ecosystem: zero-dependency CLI for Choice, Noul, Score, batch questions, and passage extraction in agent and shell pipelines.
- [CloJev](https://github.com/antlobach/clojev) — Clojure ecosystem: provides a portable Clojure client for Jev/System One calls without a Java interop layer.
...
### Local runtimes & compatible servers

- [FastJev](https://github.com/chengyongru/fastjev) — Local runtime: serves a System One-compatible API for Choice, Boolean, and Score decisions across open-model backends including Torch, vLLM, MLX, llama.cpp, and WebGPU.
- [jeff](https://github.com/logan-markewich/jeff) — Self-hosted runtime: exposes a Jev-compatible Choice, Score, and Noul API over GliFormer without a hosted Jev dependency.
...
- [jev-trust](https://pypi.org/project/jev-trust/) — Python ecosystem: wraps Jev calls with decision logging, outcome-based calibration metrics, effective-confidence annotations, overconfidence alerts, and signed evidence.
- [neurolink](https://github.com/juspay/neurolink) — Provider abstraction: exposes Jev-style `decide` as a first-class inference type alongside generate and stream in a TypeScript interface over multiple providers.
- [jev-spring-boot-starter](https://github.com/danvega/jev-spring-boot-starter) — Java ecosystem: integrates Jev with Spring Boot 4 through Spring MVC and RestClient.
- [jevify](https://github.com/altryne/jevify) — Agent skill: scans a codebase for candidate decision points, designs typed Jev questions, and packages that workflow as an installable skill.
- [discern](https://github.com/doeixd/discern) — TypeScript ecosystem: turns Jev Choice, Noul, and Score answers into typed Effect branches with caller-defined thresholds and an explicit `Uncertain` case.
...
- [mysql-ailike](https://github.com/maayanlevy/mysql-ailike) — Database integration: adds a MySQL natural-language filtering predicate powered by Jev.
